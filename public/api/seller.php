...
from tblAuction a2
left join tblAuctionExtra ae2 on a2.house=ae2.house and a2.id=ae2.id
where a2.house=results.house
and a2.item=results.item
and ifnull(ae2.bonusset,0) = ifnull(results.bonusset,0)
and a2.seller!=results.seller
and (
    (results.buy > 0 and a2.buy > 0 and (a2.buy / a2.quantity < results.buy / results.quantity)) or
    (results.buy = 0 and (a2.bid / a2.quantity < results.bid / results.quantity))
)) cheaper
EOF;

    $aucCount = 0;

    $stmt = $db->prepare('select count(*) from tblAuction where house = ? and seller = ? and item != 82800');
    $stmt->bind_param('ii', $house, $seller);
    $stmt->execute();
    $stmt->bind_result($aucCount);
    $stmt->fetch();
    $stmt->close();
    ...
    $sql = <<<EOF
select item, name, quality, class, subclass, icon, stacksize, quantity, bid, buy, `rand`, seed, bonuses, bonusurl, bonusname, bonustag, randname $cheaperSql
from (
